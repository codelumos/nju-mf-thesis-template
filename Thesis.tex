...
%% 南京大学学位论文LaTeX模板的示例文档
%% 作者：https://github.com/codelumos
%% 模版repo：https://github.com/codelumos/nju-thesis-latex-template


%% 审阅模式很重要，使用命令是下方放置在摘要后的\blind

\documentclass[winfonts,mf,twoside,AutoFakeBold= {2}]{njuthesis}
%% njuthesis 文档类的可选参数有：
%%   winfonts/linuxfonts/macfonts/adobefonts 其中 winfonts 选项使得文档使用 Windows 系统提供的字体；linuxfonts 选项使得文档使用 Linux 系统提供的字体；macfonts 选项使得文档使用 Mac 系统提供的字体；adobefonts 选项使得文档使用 Adobe 提供的 OTF 中文字体（需自行下载安转）
%%   phd/mg/mf/bachelor 选择博士、学术硕士、专业硕士、学士论文
%%   twoside/oneside 指定排版的文档为双面打印或单面打印格式（twoside 会使得每个章节从奇数页，即纸张的正面开始，因此会出现一些空白的页面）
...
% 论文的定稿日期，需设置年、月、日
% 此属性可选，若注释\date{}，则默认为最后一次编译时的日期
% 设置此属性需要同时设置英文定稿日期
% \date{2022年5月20日}
% 论文的提交日期，需设置年、月、日（仅博士）
% \submitdate{2022年5月20日}
% 学位的授予日期，需设置年、月、日（仅博士）
% \conferdate{2022年5月20日}
...
% 论文完成日期的英文形式，它将出现在英文封面下方。需设置年、月、日
% 日期使用美式日期格式，即“Month day, year”，其中“Month”为月份的英文名全称，首字母大写；
% “day”为该月中日期的阿拉伯数字表示；“year”为年份的四位阿拉伯数字表示。
% 此属性可选，若注释\englishdate{}，则默认为最后一次编译时的日期
% 设置此属性需要同时设置中文定稿日期
...
\begin{abstract}
	...
	\begin{itemize}
		...
		\item 出于严谨性的考量，本模板与使用指南目前仅对软件学院专业硕士开放（学硕如果要使用需要略作修改，可邮件联系我），请大家不要外传。
	\end{itemize}
...
% \begin{preface}
%
% 在过去的40年中，手写中文文本领域识别（HCTR）取得了很大的进展。
%
% \vspace{1cm}
% \begin{flushright}
% 某某某 \\
